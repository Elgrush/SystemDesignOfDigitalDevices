...
(
    ...
    always @ (*) begin
        case (wdSrc)
            `WD_IMMU_SRC : wd3 = immU;
            `WD_PC4_SRC  : wd3 = pcPlus4;
            `WD_AUTISM_SRC  : wd3 = { {7{sum}}, inAutism };
    ...
    always @ (*) begin
        case (pcSrc)
            `PC_BRANCH_SRC : pcNext = pcBranch;
            `PC_ALU_SRC    : pcNext = aluResult;
...
(
    ...
    always @ (*) begin
        pcSrc = `PC_PC4_SRC;
        if(branch & (aluZero == condZero)) begin
            pcSrc = `PC_BRANCH_SRC;
        end
        if(jump) begin
            pcSrc = `PC_ALU_SRC;
    ...
    always @ (*) begin
        ...
        casez( {cmdF7, cmdF3, cmdOp} )
            ...
            { `RVF7_ANY,  `RVF3_STORE_AUTISM,  `RVOP_STORE_AUTISM  } : begin regWrite = 1'b1; wdSrc = `WD_AUTISM_SRC; end
            { `RVF7_ANY,  `RVF3_SHOW_AUTISM,   `RVOP_SHOW_AUTISM  } : begin autismOut = 1'b1; end
        endcase
    end
endmodule
